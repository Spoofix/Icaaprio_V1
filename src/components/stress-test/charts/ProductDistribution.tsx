...
const tooltipContent = {
  ...
  tooltip: 'Distribution of portfolio exposure across different product types. High concentration in certain products may increase vulnerability to product-specific risks.'
};

export function ProductDistribution({ data, totalValue }: ProductDistributionProps) {
  const hhi = data.reduce((sum, item) => sum + Math.pow((item.value / totalValue) * 100, 2), 0);
