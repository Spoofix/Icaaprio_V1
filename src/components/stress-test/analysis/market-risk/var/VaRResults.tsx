...
export default function VaRResults({ results, portfolio }: VaRResultsProps) {
  const formatCurrency = (value: number) => {
    return `$${(value / 1000000).toFixed(2)}M`;
  };

  const stressLevels = [
    { name: '95% VaR', value: results.stressLevels.var95, color: '#10B981' },
    { name: '99% VaR', value: results.stressLevels.var99, color: '#F59E0B' },
    { name: '99.99% VaR', value: results.stressLevels.var9999, color: '#EF4444' }
  ];
          ...
          {stressLevels.map((level) => (
            <div key={level.name} className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
              ...
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Risk Capital: {formatCurrency(level.value * 1.5)} {/* Example capital multiplier */}
              </p>
            </div>
          ))}
        </div>
